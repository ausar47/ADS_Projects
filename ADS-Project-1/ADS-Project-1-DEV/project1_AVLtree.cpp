...
AVLtree AVL_Insert(int X, AVLtree& A) // insert in avltree
{
	if (A == NULL) { // insert directly if the tree is empty
	...
	else if (X < A->data) { // if X is smaller, insert to left
	...
	else if (X > A->data) { // if X is larger, insert to right
	...
	A->height = Max(AVL_GetHeight(A->Left), AVL_GetHeight(A->Right)) + 1; // update the height

	return A;
}

int AVL_GetHeight(AVLtree P) // get the height of the tree whose root is P
{
	if (P == NULL) return -1; // define the height of the empty tree is -1
	else return P->height;
}

AVLtree AVL_SingleRotateWithLeft(AVLtree A) // LL
{
	AVLtree K;
	K = A->Left;
	A->Left = K->Right;
...
AVLtree AVL_SingleRotateWithRight(AVLtree A) // RR
{
	AVLtree K;
	K = A->Right;
	A->Right = K->Left;
...
AVLtree AVL_DoubleRotateWithLeft(AVLtree A) // LR
{
	A->Left = AVL_SingleRotateWithRight(A->Left);
	return AVL_SingleRotateWithLeft(A);
}

AVLtree AVL_DoubleRotateWithRight(AVLtree A) // RL
{
	A->Right = AVL_SingleRotateWithLeft(A->Right);
	return AVL_SingleRotateWithRight(A);
}

int Max(int a, int b) // get the bigger one between a and b
{
	if (a > b) return a;
	else return b;
}

void nodefree(AVLtree A) // deallocating the memorise
{
	if (A != NULL) {
		nodefree(A->Left);
		nodefree(A->Right);
...
AVLtree AVL_Delete(AVLtree& A, int X) // delete node in avltree
{
	if (A == NULL) {
		return NULL;
	}
...
AVLtree AVL_getMaxNum(AVLtree A) // find the maximum node in AVLtree
{
	if (!A) {
		return NULL;
	}
...
AVLtree AVL_getMinNum(AVLtree A) // find the minimum node in AVLtree
{
	if (!A) {
		return NULL;
	}
